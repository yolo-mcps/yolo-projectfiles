...
fn get_all_processes(max_results: usize, include_full_command: bool) -> Result<Vec<ProcessInfo>, CallToolError> {
    #[cfg(target_os = "macos")]
    {
...
fn get_processes_macos(
    ...
    for (i, line) in stdout.lines().skip(1).enumerate() { // Skip header
        ...
        if parts.len() >= 5 {
            ...
            let memory_kb: f64 = parts[3].parse().unwrap_or(0.0);
...
fn get_full_command_windows(pid: u32) -> Result<String, std::io::Error> {
    ...
    let output = Command::new("wmic")
        .args(&["process", "where", &format!("ProcessId={}", pid), "get", "CommandLine", "/format:value"])
        .output()?;
...
fn get_process_name_by_pid_windows(pid: u32) -> Result<String, std::io::Error> {
    ...
    Err(std::io::Error::new(std::io::ErrorKind::NotFound, "Process name not found"))
}
