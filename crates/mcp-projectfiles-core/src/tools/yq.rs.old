...
#[mcp_tool(name = "yq", description = "Query and manipulate YAML files using jq-style syntax.

Supports both read and write operations:
- Read operations: Query YAML data and return results in various formats
- Write operations: Apply transformations and save changes back to file

Examples:
- Query a field: {\"file_path\": \"config.yaml\", \"query\": \".database.host\"}
- Array access: {\"file_path\": \"data.yaml\", \"query\": \".users[0]\"}
- Simple assignment: {\"file_path\": \"config.yaml\", \"query\": \".debug = true\", \"operation\": \"write\", \"in_place\": true}
...
impl YamlQueryTool {
    ...
    fn execute_query(&self, data: &serde_json::Value, query: &str) -> Result<serde_json::Value, YamlQueryError> {
    ...
    fn simple_path_query(&self, data: &serde_json::Value, query: &str) -> Result<serde_json::Value, YamlQueryError> {
...
impl StatefulTool for YamlQueryTool {
    async fn call_with_context(
        ...
        let canonical_path = if self.operation == "read" || (self.operation == "write" && file_path.exists()) {
            ...
            if !file_path.exists() {
                return Err(CallToolError::from(tool_errors::file_not_found("yq", &self.file_path)));
            }
        ...
        // Read the YAML file
        let mut data = self.read_yaml_file(&file_path).map_err(|e| CallToolError::from(tool_errors::invalid_input("yq", &e.to_string())))?;
        ...
        let result = match self.operation.as_str() {
            "read" => {
                ...
                self.execute_query(&data, &self.query).map_err(|e| CallToolError::from(tool_errors::invalid_input("yq", &e.to_string())))?
            }
            "write" => {
                ...
                if self.in_place {
                    ...
                    if let Some((path, value)) = self.parse_assignment(&self.query).map_err(|e| CallToolError::from(tool_errors::invalid_input("yq", &e.to_string())))? {
                        self.apply_assignment(&mut data, &path, value).map_err(|e| CallToolError::from(tool_errors::invalid_input("yq", &e.to_string())))?;
                        modified = true;
                        
                        // Write the modified data back to file
                        self.write_yaml_file(&file_path, &data, self.backup).map_err(|e| CallToolError::from(tool_errors::invalid_input("yq", &e.to_string())))?;
                        data.clone()
                    } else {
