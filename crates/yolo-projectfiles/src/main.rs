...
#[derive(Parser)]
#[command(name = "yolo-projectfiles")]
#[command(author, version, about = "YOLO ProjectFiles MCP Server", long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    Stdio {
        /// Server name
        #[arg(long, default_value = "yolo-projectfiles")]
        name: String,
...
async fn main() -> Result<()> {
    ...
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("mcp_projectfiles_core=info,yolo_projectfiles=info,info"));
    ...
    match cli.command {
        Commands::Stdio { name: _, version: _, project_root } => {
            ...
            info!("Starting YOLO ProjectFiles MCP server with stdio transport");
