...
#[derive(Parser)]
#[command(name = "yolo-terminator")]
#[command(author, version, about = "YOLO Terminator MCP Server - Process Termination", long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    Stdio {
        /// Server name
        #[arg(long, default_value = "yolo-terminator")]
        name: String,
...
async fn main() -> Result<()> {
    ...
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("mcp_projectfiles_core=info,yolo_terminator=info,info"));
